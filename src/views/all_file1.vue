<template>
    <div class="container all_file_container">
        ...
        <div class="file_body clearfix" :class="{'search_file_body':isShowPath}">
            <!-- 表格列表 -->
            <el-col v-if="isShowtable" :span="24" v-loading="isloadlist">  
                <el-table
                    ref="multipleTable"
                    :data="tableData"
                    tooltip-effect="dark"
                    ...
                    <el-table-column type="selection" width="55" label="全选" :selectable="handleFileroot"></el-table-column>
                    <el-table-column prop="fileName" label="文件名"  min-width="200" :sortable="'custom'">
                        <template slot-scope="scope">
                            <div class="flex-wrap flex-con filename_td" >
                                ...
                                <img class="filename_icon fl" v-if="scope.row.suffix=='.xls'||scope.row.suffix=='.xlsx'||scope.row.suffix=='.xlsm'||scope.row.suffix=='.xltx'||
                                ...
                                <span class="table_filename" style="" @click="dblclickFileRow(scope.row)">{{ scope.row.fileName }}</span>
                                ...
                                <!-- 是否收藏操作 -->
                                <img v-if="isRbac_collect && scope.row.authority == 1 && scope.row.collected == 0" style="cursor:pointer;" class="table_fileroot" src="../assets/icon_sc_no.png" @click.stop="handleSetCollectwj(scope.row)">
                                <img v-if="isRbac_collect && scope.row.authority == 1 && scope.row.collected == 1" style="cursor:pointer;" class="table_fileroot display" src="../assets/icon_sc_hs.png" @click.stop="handleSetCollectwj(scope.row)">
                            </div>
                    ...
                    <el-table-column label="操作" width="150" v-if="!isShowPath">
                        <template slot-scope="scope">
                            <div v-if="isRbac_down" class="table_cz_icon" @click="handledownloadWj(scope.row)"><img src="../assets/icon_xz.png"></div>
                ...
                <div class="thumbnail_list clearfix">
                    <div class="thumbnail_item"
                        ...
                        <div class="thumbnail_item_top clearfix" v-show="currthumbindex==index || item.isChecked">
                            <img v-if="item.authority == 1" class="radio_img fl" src="../assets/icon_check_radiao.png" :class="{'opcity':item.isChecked}" @click.stop="spikthumbRadio(item,index)">
        ...
        <el-dialog
            ...
            <el-form :model="ruleFormWjj" :rules="wjjrules" ref="ruleFormWjj" label-width="100px" class="demo-ruleForm">
                <el-form-item label="文件夹名" prop="fileName">
                    <el-input placeholder="请输入文件名" v-model.trim="ruleFormWjj.fileName"></el-input>
                </el-form-item>
        ...
        <el-dialog
            ...
            <div class="root_head clearfix">
                    ...
                    <div class="upload-right flex-con">
                       ...
                       <div class="" style="width:100%;margin-top:5px;">
                           ...
                           <el-progress :percentage="percentage_num" v-if="percentage_num > 0 && percentage_num <= 100"></el-progress>
                            <div v-if="percentage_num > 0 && percentage_num <= 100" style="font-size:12px;color:#999;line-height:12px;margin-bottom:10px">{{files_all.length}}/{{files_all_length}}</div>
                            <!-- <div style="font-size:12px;color:#999;line-height:12px;">{{chunksize_all}}/{{filesize_all}}</div> -->
                           <ul class="upload_ul">
                               <!-- -->
                ...
                <div class="upload_item flex-wrap">
                    ...
                    <div class="upload-right flex-con">
                        <textarea class="upload-r-textarea" name="" id="" cols="30" rows="10" v-model="upload_description"></textarea>
                    </div>
                </div>
            </div>
            <div class="root_center">
  ...
  methods: {
    ...
    inittable() {
        let param={
            ...
            'empAccId':this.empAccId,
            'fileTarId':this.fileTarId || this.$route.query.fileTarId,
            'page':this.now_page,
            'size':this.page_size,
        }
    ...
    ajaxAallDirectories(){
        ...
        getData("web/file/empAcc/allDirectories", {
            ...
            if (res.code == 200) {
                const data = res.data || [];
                //对选中文件夹进行过滤
                // that.allDirectories=data;
                // console.log(that.r_fileArr)
                // console.log(JSON.stringify(data,null,'\t'))
                // that.allDirectories = data.filter((item,index) =>{
                ...
                // })
                that.allDirectories=data
                that.allD_result = construct(that.allDirectories, {
                    id: 'fileTarId',
                    pid: 'parentFileId',
                    children: 'children'
                })
                //对过滤后的文件夹数据进行没有权限的文件夹进行过滤
                // console.log(JSON.stringify(that.allD_result,null,'\t'))
                // const rArr = that.allD_result.reduce((init,value) => {
                //     init.push(that.fnFormatData(value))
                //     return init;
                // },[]);
                // // console.log(JSON.stringify(rArr,null,'\t'))
                // that.res_id = rArr
            }
        })
    },
    ...
    selectFile(event) {
        ...
        let time,that = this,fileLength=event.target.files.length,all_file_length=1;
    ...
    xhrDownloadWj(row){
        let that=this;
        let ajaxUrl = axios.defaults.baseURL+'web/file/download?empAccId='+this.empAccId+'&fileTarIds='+this.download_fileTarIds+'&fileName='+row.fileName;
        ...
        xhr.onprogress = function (event) {
            const progress = parseInt(event.loaded / row.size * 100)
    ...
    handleShareComfrim(e){
        ...
        getData("web/file/share", {
            ...
            if (res.code == 200) {
                // that.shareUrl = arrUrl[0]+'/#/link_share';
                that.shareUrl = res.data;
                that.$copyText(that.shareUrl).then(function (e) {
                    Dialog.message('生成链接成功，去复制粘贴分享吧!','success',3000)
                })
                // that.isShareWjjdialog=false;
            }else{
                Dialog.message('分享失败!','error',1500)
            }
            setTimeout(() => {
    ...
    handleSetRootwj(row,iconPath){
        ...
        this.root_title = '设置'+row.fileName+'文件（夹）权限';
    ...
    handlethumbCheckAll(val) {
        ...
        if(this.thumb_checkAll){
            this.tableData.map(function(v,i){
                if(v.isChecked == false && v.authority == 1){
                    v.isChecked = true
                }
                if(v.authority == 1){

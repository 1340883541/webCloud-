    ...
    <div class="container all_file_container">
                ...
                <div class="file_h_left" v-if="!isShowPath">
                    <div class="file_h_item" v-if="isRbac_upload">
                        <el-dropdown placement="bottom-start" trigger="click"  v-if="isuploadWjj">
                            ...
                            <el-button type="primary" size="small"@click.native="handleUploadWj" v-else>
                                <img src="../assets/icon_up.png">上传
                            </el-button>
                    </div>
                    ...
                    <div class="file_h_item" v-if="isRbac_fileRoots && isAuth">
        ...
        <el-dialog
            ...
            <div class="root_head clearfix">
                <div class="upload_item flex-wrap">
                    ...
                    <div class="upload-right flex-con">
                        <div class="upload-open" @drop.prevent="fileDrop" @dragover.prevent="fileDragover" @click="fileClick">
                       ...
                       <div class="cleafix" style="width:100%;margin-top:5px;">
                           ...
                           <!-- <el-progress :percentage="percentage_num" v-if="percentage_num > 0 && percentage_num <= 100"></el-progress> -->
                            ...
                            <!-- 3.0取消上传框监控<div  v-if="percentage_num > 0 && percentage_num <= 100 && !isuploadmsg" class="fl upload_numdiv">已上传文件: {{files_all.length}}/{{files_all_length}}</div>
                           ...
                           <ul class="upload_ul">
                               <!-- -->
                               <li class="flex-wrap flex-align-item flex-hh-not" v-for="(item,index) in beforeupload" :key="index"
                                @mouseenter="spikEnterUploadli(index)" @mouseleave="spikLeaveUploadli">
                                    <p class="flex-con"><img class="fl" src="../assets/icon_attached.png"> <span class="elipseone">{{item.files.name}}</span></p>
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button v-if="beforeupload.length> 0" size="small" type="primary" @click="startUpload">开始上传</el-button>
                <el-button v-if="beforeupload.length <= 0" size="small" type="primary" disabled>开始上传</el-button>
...
import {Dialog, unique,myBrowser,api,getCookie} from "../api/util.js";
...
export default {
  ...
  data() {
    ...
    return {
        ...
        uploadXhr :[],
        timer:'',
        allfile:downloadStore.state.message.upload_p_list,//存储全部文件
        lineupfile:[],//正在排队的文件
        suspendfile:[],//暂停的文件
        uploadingfile:[],//正在上传文件
        tempuploadfild:[],//临时存储排队文件
        beforeupload:[],//本次上传文件数组
        loadnum:0,//存储上传个数           
    };
  },
  mounted() {
  ...
  watch:{
      lineupfile:{
        handler(val1,val2){
          var that = this;
    if(this.tempuploadfild.length>0){
        uploadFile( this.tempuploadfild[0],this.tempuploadfild[0].num, this.empAccId, this.corpAccId, this.parentFileId, 0,(files,len,progress,isLast,response,chunkSeq) => {
                    // upload-success
                    ...
                    if(responseJson.code == 200){
                        ...
                        if(responseJson.data.folderId){
                            that.filejiaId = [responseJson.data.folderId]
                        }
                        ...
                        that.lineupfile.length>0 && responseJson.data.process==100 ? that.tempuploadfild.push(that.lineupfile.shift()):'';
                    }else{
  ...
  methods: {
    ...
    fileClick() {
        ...
        // progressArrFunc([])
        this.upload_speed='0.01M/s'
        this.percentage_num=0
        this.files_all_length=this.files_all.length
    },
    selectFile(event,drag) {
        ...
        let that = this;
         var files = drag?event.dataTransfer.files:event.target.files ;
        //文件夹上传验证重名接口
        if(this.is_upload_wj == 0){
            ...
            this.isUploadFunc(event,drag)
        }
        // 重置file input控件的值
        // event.target.value = ''
    },
    //文件（夹）上传前期准备
    isUploadFunc(event,drag){
        let that = this,totalSize = 0, g_size = api.g_size, z_size=1073741824 // 1G;
      ...
      let isVideo =typeList.some((item)=>{
        ...
        if(totalSize < g_size){
            if(isVideo){
                if(totalSize > z_size){
                    mssageTxt ='文件大小已超1G，超大文件的上传可能会很慢哦，需耐心等待，您确定上传么?视频文件建议MP4格式。如其他格式则将自动进行转码可能会存在画面失真情况。'
                }else{
                    mssageTxt ='视频文件建议MP4格式。如其他格式则将自动进行转码可能会存在画面失真情况。'
                }
                that.$confirm(mssageTxt, '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                type: 'warning'
                     }).then(() => {
                ...
                if(totalSize > z_size){
                    that.$confirm('文件大小已超1G，超大文件的上传可能会很慢哦，需耐心等待，您确定上传么?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
                    ...
                    that.ajaxUpload(event,drag)
                    that.isuploadmsg = false;
                }
            }
        
        }else{
            Dialog.message('请选择小于10G的文件进行上传!','error',2000);
            that.isuploadmsg = false;
            that.percentage_num = 0
            event.target.value = ''
        }
    },
    ajaxUpload(event,drag){
        var that = this;
        var files = drag?event.dataTransfer.files:event.target.files ;
        this.files_all_length = files.length+this.files_all.length;
        if(this.files_all_length <= 1000){
         for (let i =0; i < files.length; i++) {
         var file =files[i];
            getFileMD5(file,(md5)=>{
                   var isrepeat2 = downloadStore.state.message.upload_p_list.some((item)=>{
                        console.log(item.type )
                        return (item.md5 == md5 && item.type != 1);
                    })
                       var isrepeat1 =  that.beforeupload.some((item)=>{
                           return item.md5 == md5;
                        })
                        if(isrepeat1){
                            Dialog.message('待上传列表已经存在这个'+file.name+'文件了!','error',2000);
                            return
                        }
                    if(isrepeat2){
                        Dialog.message('上传列表已经有相同任务正在进行了!','error',2000);
                        return
                     }
                    ...
                    var  chunks = Math.ceil(file.size / chunkSize) > 0 ? Math.ceil(file.size / chunkSize) : 1;
                        this.loadnum = downloadStore.state.message.upload_p_list.length;
                    this.beforeupload.push({
                        files:file,
                        slices:chunks,
                        uploadslices:0,
                        progress:0,//上传进度
                        uploadSpeed:0,//上传速度
                        uploadSize:0,//已上传大小
                        type:0, // 0未上传 1上传成功 2上传中 1001 重名 1002 暂停
                        num:i+this.loadnum,
                        md5,md5,//
            ...
            function getFileMD5(file,callback){
                 var fileReader = new FileReader();
                    ...
                    //每块交由sparkMD5进行计算
                    console.log(i)
                    var md5_wj = that.empAccId+that.parentFileId +'0'+ file.name + file.webkitRelativePath+ e.target.result;
    ...
    startUpload(){
        ...
        downloadStore.state.message.is_tab_progress=1; 
        downloadStore.state.message.upload_p_list=  downloadStore.state.message.upload_p_list.concat(this.beforeupload)
        if( downloadStore.state.message.upload_p_list.length>0){
             uploadingArr=  downloadStore.state.message.upload_p_list.filter((item)=>{
                return item.type ==2
            })
        }
        var uploadingLen = uploadingArr.length;
            this.percentage_num = 1
            this.filesize_all = 0
            for (let i =0; i < this.beforeupload.length; i++) {
      
             let file =  this.beforeupload[i].files;
             if(file.size>1073741824){
                    var chunkSize = 5242880; // 10M 10485760 50M 52428800;
                    }else{
                    var chunkSize = file.size > 52428800 ? 2097152 : 52428800; // 10M 10485760 50M 52428800;
                ...
                var  chunks = Math.ceil(file.size / chunkSize) > 0 ? Math.ceil(file.size / chunkSize) : 1
                that.filesize_all = that.filesize_all+this.beforeupload[i].size
                // console.log(that.filesize_all)
                // 调用上传方法，传入选择的文件对象
                if(i<5-uploadingLen){
                    that.uploadingfile.push({
                        files:this.beforeupload[i].files,
                        slices:chunks,
                        uploadslices:0,
                        uploadSize:0,//已上传大小
                        uploadSpeed:0,//上传速度
                        ...
                        type:0, //未上传
                        num:i,
                        md5:this.beforeupload[i].md5,//
                        parentFileId:that.parentFileId,
                        where:0,
                    })
                 uploadFile(this.beforeupload[i],downloadStore.state.message.upload_p_list[i+this.loadnum].num, this.empAccId, this.corpAccId, this.parentFileId, 0 
                 ,(files,len,progress,isLast,response,chunkSeq) => {
                    // upload-success
                    let responseJson = eval('(' + response + ')');
                    if(responseJson.code == 200){
                        // console.log(files)
                       ...
                       if( responseJson.data.process==100){
                             downloadStore.state.message.upload_p_list[len].type =1 //上传完成状态
                             console.log(len,'+++',downloadStore.state.message.upload_p_list)
                       }
                        downloadStore.state.message.upload_p_list[len].uploadslices =chunkSeq;
                        that.lineupfile.length>0 && responseJson.data.process==100 ? that.tempuploadfild.push(that.lineupfile.shift()):'';
                    }else{
                    ...
                    if(status == 0){
                        ...
                        --that.files_all_length;
                        that.beforeupload = [];
                         that.beforeupload.push(downloadStore.state.message.upload_p_list[len])
                        downloadStore.state.message.upload_p_list[len].type =1001
                    }
                },(speed,timer,len)=>{
                    //上传文件网速
                    // console.log(speed + " KB/s");
        ...
        //保存接口
        var fileMd5s =  this.beforeupload.map((item)=>{
            return item.md5;
        })
        let folderMd5=downloadStore.state.message.folderMd5;
        let param={
        ...
        if(this.beforeupload.length > 0){
            // if(this.files_all.length == this.files_all_length){
                ...
                getData("web/file/filesAuthSave", param).then(res => {
                    if (res.code == 200) {
    ...
    spikUploadDel(item,index){
        let that = this;
        this.beforeupload.splice(index,1);
    ...
    handledownloadWjHead(){
        if(this.download_fileTarIds.length > 0){
          ...
          downloadStore.state.message.is_tab_progress=0;
          downloadStore.state.message.is_progress=true;
    },
    ...
    dblclickFileRow(row){
        ...
        if(row.type == 1){
           ...
           //视频
        if(row.type == 4){
        // var sourcesType ="video/"+row.suffix.substring(1,row.suffix.length)
    ...
    docPreview(row){
        getData("web/file/preview", {
            ...
            setTimeout(() => {
                ...
                //     //window.open(url,'_blank',"menubar=0,scrollbars=1, resizable=1,status=1,titlebar=0,toolbar=0,location=1");
                //     //使用window.open方法打开的窗口可能被拦截的替代方案，可利用超链接打开的url是不会被浏览器拦截
                //     // this.$refs.iframeView.src='http://192.168.110.92:8012/onlinePreview?url='+encodeURIComponent(url)
